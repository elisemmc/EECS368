...
(define (symderiv2 sf var n)
  (list 'λ var (nth-symderiv sf (car var) n)))

(define (Usymderiv sf)
  (list 'λ (cadr sf)(symderivat (caddr sf)(caadr sf))))  
...
(define (symderiveat function variable)
  (cond
    ...
    (else(case(car function)
           ...
           ((+) 
               (add 
                    ...
                    (if(=(length function)3)
                       ...
                       (symderiveat (+fix(cons '+ (cddr function))) variable))
                    ))
           ...
           ((-) 
               (subtract 
                     ...
                     (cond
                            ...
                            ((>(length function)3) (symderiveat (+fix(cons '+ (cddr function))) variable))
                            )))
           ...
           ((asin)  
              (*fix(list '* (symderiveat (cadr function) variable) (list '/ 1 (list 'sqrt (subtract 1 (*fix(list '* (cadr function)(cadr function)))))))))
           
           ((acos) 
              (list '- (*fix(list '* (symderiveat (cadr function) variable) (list '/ 1 (list 'sqrt (subtract 1 (*fix(list '* (cadr function)(cadr function))))))))))
           
           ((atan)  
              (*fix(list '* (symderiveat (cadr function) variable) (list '/ 1 (+fix(list '+ 1 (*fix(list '* (cadr function)(cadr function)))))))))
           ...
           ((expt)         
              (cond
                 ...
                 (else
                   (list '* function 
                         (+fix(list '+ 
                               (*fix(list '* (symderiveat (cadr function) variable)(list '/ (caddr function) (cadr function))))
                               (*fix(list '* (symderiveat (caddr function) variable)(list 'log (cadr function))))))))
                 ))
...
(define (quotient_rule num den variable) 
   (list '/ 
         (subtract (*fix(list '* den (symderiveat num variable))) (*fix(list '* (symderiveat den variable) num)))
         (*fix(list '* den den))))
...
(define (nth-symderiv function x n)
  (if(eq? 0 n)
     function
     (if(eq? 1 n)
        (symderiveat function x)
        (nth-symderiv (symderiveat function x) x (- n 1)))
  ))

(define (nth-symderivU function x n)
  (if(eq? 0 n)
     function
     (nth-symderivU (symderivat 'x function) 'x (- n 1)))
  )

(define (nth-deriv function xVal n)
  (if(eq? 0 n)
     (function xVal)
     (nth-deriv (numdef function) xVal (- n 1)) ))

(define (to-func input)(eval(list 'λ '(x) input)env)) 
