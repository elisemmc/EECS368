...
(define (symderiveat function variable)
  (cond
    ...
    (else(case(car function)
           ...
           ((-) 
               (if (=(length function)2) (list '- (symderiveat(cadr function) variable))
                     (list '- (symderiveat (cadr function) variable)
                     (cond
                            ((=(length function)2) '☃)
                            ((<(length function)3) 0)
           ...
           ((*) 
                (cond
                    ...
                    (else
                       (list '+ 
                          (list '* (symderiveat(cadr function) variable) (append '(*) (cddr function)))
                          (list '* (cadr function) (symderiveat(append '(*) (cddr function)) variable))
                 ))))
           ...
           ((/) 
              (cond
                  ...
                  ((>(length function)2) (quotient_rule (cadr function) (append '(*) (cddr function)) variable))
                  ))
           
           ((exp) 
              (list '* (symderiveat (cadr function) variable) function))
           
           ((sin) 
              (list '* (symderiveat (cadr function) variable) (list 'cos (cadr function))))
           
           ((cos) 
              (list '* (symderiveat (cadr function) variable) (list '- (list 'sin (cadr function)))))
           
           ((tan) 
              (list '* (symderiveat (cadr function) variable) (list '/ 1 (list '* (list 'cos (cadr function))(list 'cos (cadr function))))))
           
           ((asin)  
              (list '* (symderiveat (cadr function) variable) (list '/ 1 (list 'sqrt (list '- 1 (list '* (cadr function)(cadr function)))))))
           
           ((acos) 
              (list '- (list '* (symderiveat (cadr function) variable) (list '/ 1 (list 'sqrt (list '- 1 (list '* (cadr function)(cadr function))))))))
           
           ((atan)  
              (list '* (symderiveat (cadr function) variable) (list '/ 1 (list '+ 1 (list '* (cadr function)(cadr function))))))
           ...
           ((expt)         
              (cond
                 ...
                 ((number? (cadr function)) (list '* (symderiveat (caddr function) variable) function (list 'log (cadr function))))
                 (else
                   (list '* function 
                         (list '+ 
           ...
           ((abs) (list '* (symderiveat (cadr function) variable) (list '/ (cadr function) function)))
...
(define (quotient_rule num den variable) 
   (list '/ 
         (list '- (list '* den (symderiveat num variable)) (list '* (symderiveat den variable) num))
         (list '* den den)))

;This function subtracts
(define (subtract f s)
  (cond ((equal? s '☃) (list '- f))
        ((and (number? f)(= f 0)) (list '- s))
        ((and (number? s)(= s 0)) f)
